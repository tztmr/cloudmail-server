...
export async function handleReceivedEmail({ env, rawContent, to, onReject, onForward } = {}) {
	if (!env) {
	...
	let rejectReason = '';
	const raw = new Response(rawContent).body;
	const message = {
		to,
		raw,
		setReject(reason) {
			rejectReason = reason || 'Message rejected';
		...
		async forward(target) {
			if (typeof onForward === 'function') {
				return onForward(target);
			}
			console.warn(`跳过邮件转发 ${target}：服务器模式未配置转发实现`);
		}
	};

	await email(message, env, {});
	return { rejected: !!rejectReason, reason: rejectReason };
}
